...
namespace LL.Core.Common.Helpers
{
    /// <summary>
    /// 类型转换
    /// </summary>
    public static class ConvertHelper
    {
        ...
        /// <summary>
        /// 转换为可空日期 可转换格式为 2020/08/03/周一 12:00:00
        /// </summary>
        /// <param name="input">输入值</param>
        public static DateTime? ToDateTimeOrNull(object input)
        {
            string v = input.SafeString();
            ...
            if (v.IndexOf("/") > 0)
            {
                ...
                string s = $"{ss[0]}-{ConvertHelper.ToInt(ss[1]).ToString("00")}-{ConvertHelper.ToInt(ss[2]).ToString("00")}";
                if (items.Length == 2)
                {
                    return ToDateOrNull($"{s} {items[1]}");
                }
                else
                {
        ...
        ///  <returns></returns>
        public static string[] Split(string req, char separator = ',')
        {
            if (string.IsNullOrEmpty(req))
                return null;
            else
            {
                return req.Split(separator);
            }
        }
        /// <summary>
        /// 获取可能为null的double数据并转成string
        /// </summary>
        /// <param name="v"></param>
        /// <returns></returns>
        public static string GetNullDouble(double? v)
        {
            if (v == null)
            {
                return null;
